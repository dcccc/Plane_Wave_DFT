...
def scf(V_loc_r, psi_g, rho_r_all, beta_nl, g_vector, input, libxc, 
          is_hf=False, is_hybrid=False, exx_alpha=0.0):
    ...
    is_gga = is_gga > 0
    # print("is_gga:", is_gga, " is_need_lap:", is_need_lap, " is_need_tau:", is_need_tau, " is_hybrid:", is_hybrid)
    # print("scf exx_alpha:", exx_alpha)
    ...
    if is_hf or is_hybrid:    
        phi_ia = get_phi(psi_g, op_coul, vol) *  np.prod(grid_point)**2

    rho_value = [rho_r_all, rho_grad_r, kden, rho_lap_r]
    # eval0, psi_g_1d = diag_davidson(V_loc_r, psi_g, rho_value, beta_nl, g2_vector, g1_vector, op_coul, \
    #               grid_point, gx_vector_mask, n_state_all, ps_list, atom_symbol, 
    #               vol, state_occupy_list, libxc, phi_ia, exx_alpha=exx_alpha)

    eval0, psi_g_1d =  diag_cg(V_loc_r, psi_g, rho_value, beta_nl, g2_vector, g1_vector, op_coul, \
                  grid_point, gx_vector_mask, n_state_all, ps_list, atom_symbol, vol, state_occupy_list, \
                  libxc, phi_ia,exx_alpha=exx_alpha)
    ...
    E_pw = cal_E_total(psi_g_3d, V_loc_r, rho_value, beta_nl, input, g_vector, libxc=libxc, exx_alpha=exx_alpha)
    ...
    for ii in range(80):
        ...
        # eval1, psi_g_1d = diag_davidson(V_loc_r, psi_g_3d, rho_value, beta_nl, g2_vector, g1_vector, op_coul, grid_point,\
        #                                 gx_vector_mask, n_state_all, ps_list, atom_symbol, 
        #                                 vol, state_occupy_list, libxc, phi_ia, exx_alpha=exx_alpha)


        eval1, psi_g_1d =  diag_cg(V_loc_r, psi_g_3d, rho_value, beta_nl, g2_vector, g1_vector, op_coul, \
                  grid_point, gx_vector_mask, n_state_all, ps_list, atom_symbol, vol, state_occupy_list, libxc, phi_ia, 
                  exx_alpha=exx_alpha)
        ...
        if is_hf or is_hybrid:
            phi_ia = get_phi(psi_g_3d, op_coul, vol) *  np.prod(grid_point)**2

        deval = np.mean(np.abs(eval1 - eval0))  
        eval0 = eval1

        # calculate total band energy
        
        rho_value = [rho_new, rho_grad_r, kden, rho_lap_r]
        E_pw_new = cal_E_total(psi_g_3d, V_loc_r, rho_value, beta_nl, input, g_vector, libxc, exx_alpha=exx_alpha)
        ...
        # if ii >= 10 :
        #     rho_new = pulay_mix(rho_list, r_list, beta = 0.1 )
        ...
        # print("eval:", np.around(eval1, decimals=6))
        if dE_total < 0.0000001 and deval< 0.0000001 and d_rho < 0.001:
            is_converged = True
            break    


    return(eval1, psi_g_3d, rho_new, E_pw_new , is_converged)
